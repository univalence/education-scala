  ...
  section("ADT") {
    ...
    exercise("Evaluate an expression") {
      ...
      exercise("Simplify an expression") {
         ...
         * function to be recursive, in a view to explore all the parts
         * of your expression.
         *
         * Also, just one pass (ie. one recursive call) might not be
         * enough to propagate your rules on the whole expression. For
         * example, even a recursive call will not simplify the
         * expression `Mult(Variable, Mult(Variable, Zero))` into
         * `Zero`. At best, you will get `Mult(Variable, Zero)`. So, in
         * this exercise, you will need the function [[fixedPoint]]
         * below, that will apply your simplification function until, no
         * more simplification happens.
         */
         ...
         *
         * This function will find the value of `x` such that `f(x) ==
         * x`. A `limit` is given in a view to avoid infinite loops.
         *
         * @param f
